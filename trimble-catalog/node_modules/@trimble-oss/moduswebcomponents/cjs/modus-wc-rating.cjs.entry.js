...
const ModusWcRating = class {
    constructor(hostRef) {
        ...
        this.value = 0;
        this.uniqueRatingGroupName = `modus-wc-rating-group-${utils.generateRandomId(4)}`;
    }
    componentWillLoad() {
    ...
    renderRatingItems(ratingItemClasses) {
        return Array.from({ length: this.getTotalRatingItems() }, (_, index$1) => {
            const ratingValue = this.getValueForIndex(index$1);
            const itemClass = getIndexedRatingItemClass(index$1, ratingItemClasses, this.supportsHalfRatings(), this.variant, this.count);
            return (index.h("input", { "aria-label": this.getAriaLabelText(ratingValue), "aria-checked": this.value === ratingValue ? 'true' : 'false', checked: this.value === ratingValue, class: itemClass, disabled: this.disabled, key: index$1, name: this.uniqueRatingGroupName, onChange: () => this.handleChange(ratingValue), type: "radio", value: String(ratingValue) }));
        });
    }
    /**
     ...
     */
    renderZeroOption() {
        return (index.h("input", { "aria-label": this.getAriaLabelText(0), "aria-checked": this.value <= 0 ? 'true' : 'false', checked: this.value <= 0, class: "modus-wc-rating-hidden", disabled: this.disabled, name: this.uniqueRatingGroupName, onChange: () => this.handleChange(0), type: "radio", value: "0" }));
    }
    render() {
        const { ratingClasses, ratingItemClasses } = this.getClasses();
        return (index.h(index.Host, { key: '91b42fb9bff7e60cf871eccf6c9e3509629d1094', class: "modus-wc-rating-container" }, index.h("div", Object.assign({ key: '87c404d5ac3a5c4ab1274fbeeb56e457c63b76cb', class: ratingClasses, role: "radiogroup" }, this.inheritedAttributes), this.renderZeroOption(), this.renderRatingItems(ratingItemClasses))));
    }
    get el() { return index.getElement(this); }
};
ModusWcRating.style = modusWcRatingCss;

exports.modus_wc_rating = ModusWcRating;
